...
#' 
#' @export
isoPLSDA <- function(variables, group, validation = NULL, learn = NULL, test = NULL, tol = 0.001, nperm = 400, refinment = FALSE, vip = 1.2){

	# Auxiliar data containing variable names and numeric ID per variable
	dataVariables <- data.frame( variable = colnames(variables), id = c(1:dim(variables)[2]))

	# PLS-DA model
	model.plsDA <- plsDA(variables, group, validation, learn, test)

	# R-squared coefficents (number of components are selected based on cumulative R-squared coefficient changes)
	...
	R2.perm <- R2PermutationVector(variables, group, validation, learn, test, tol, nperm)
	...
	if(refinment == TRUE){
		...
		dataVIP <- data.frame(variable = row.names(model.plsDA$VIP), VIP = vip.max)
		dataVIPref <- dataVIP[dataVIP$VIP >= vip,]
		sel.vars <- merge(dataVariables , dataVIPref, by.x="variable", by.y="variable")
		...
		R2.perm.ref1 <- R2PermutationVector(variables.ref, group, validation, learn, test, tol, nperm)
		p.val.ref1 <- sum(R2.mat.ref[dim(R2.mat.ref)[1],4] <= R2.perm.ref1, na.rm=TRUE)/nperm

		# p-value based on performing the whole refinement process in  each permutation iteration
		R2.perm.ref2 <- R2RefinedPermutationVector(variables, group, validation, learn, test, tol, nperm, vip)
		p.val.ref2 <- sum(R2.mat.ref[dim(R2.mat.ref)[1],4] <= R2.perm.ref2, na.rm=TRUE)/nperm
...
#' Function that computes p-values when only individuals are considered to be permuted
#' @param variables   matrix or data frame with explanatory variables
#' @param group   	vector or factor with group memberships
#' @param validation	type of validation, either NULL or "learntest". Default NULL
#' @param learn		optional vector of indices for a learn-set. Only used when validation="learntest". Default NULL
...
R2PermutationVector <- function(variables, group, validation, learn, test, tol, nperm){
...
#' Function that computes p-values when a refinment process is considered per each permutation iteration
#' Arguments (included in plsDA function within DiscriMiner package)
#' @param variables   matrix or data frame with explanatory variables
#' @param group   	vector or factor with group memberships
#' @param validation	type of validation, either NULL or "learntest". Default NULL
...
#' @return PLS model
#' 
R2RefinedPermutationVector <- function(variables, group, validation, learn, test, tol, nperm, vip){
	...
	for(p in 1:nperm){
		...
		dataVIP.perm <- data.frame(variable = row.names(model.perm$VIP), VIP = vip.perm.max)
		dataVIP.perm.ref <- dataVIP.perm[dataVIP.perm$VIP >= vip,]
		sel.vars.perm <- merge(dataVariables , dataVIP.perm.ref, by.x="variable", by.y="variable")
		variables.perm.ref <- variables[,sel.vars.perm[,2]]

		if(!is.null(dim(variables.perm.ref)[2])){
			model.perm.ref <- plsDA(variables.perm.ref, group, autosel = TRUE, comps = 2, validation, learn, test)
...
###############################################################################################################################################
########################################       FUNCTION THAT PERFORMS A LATTICE PLOT       ####################################################
###############################################################################################################################################

#' Components plot (LATTICE PLOT)
#' @aliases isoPLSDAplot
#' @usage isoPLSDAplot(components, groups)
#' @param components   PLS-DA components as it comes from plsDAres main function
#' @param group   	vector or factor with group memberships
#' @return plot
#' 
#' @export
