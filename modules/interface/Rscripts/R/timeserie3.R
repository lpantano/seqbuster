		...
		for (j in 2:ns){
			...
			##td<-newXMLNode("td",attrs=c(bgcolor="grey"),tableref[i,j+1] ,parent = tr)
					...
					range<-max(exp2,exp1)
							...
							y0<-0.10
							y1<-(exp2/range)*100
							ye1<-((exp1)/(rangeall))*100
							ye2<-((exp2)/(rangeall))*100
							pointsx<-c(pointsx,y1-y2)
							if (table[i,j]<0.05){
								pointsc<-c(pointsc,"red")
							}else{
								pointsc<-c(pointsc,"black")
							}
					...
					exp1<-as.numeric(ref[j])
					exp2<-as.numeric(ref[j+1])
					range<-max(exp2,exp1)
						ye1<-((exp1)/(rangeall))*100
						ye2<-((exp2)/(rangeall))*100
						...
						y0<-0.50
						y1<-(exp2/range)*100
						pointsy<-c(pointsy,y1-y2)
						##segments(x0,y0,x0,y2)
						##segments(x0,y0,x1,y0)
						##segments(x1,y0,x1,y1)
						##segments(x0,y2,x1,y1)
						##Spolygon(c(x0,x1,x1,x0),c(y0,y0,ye2,ye1),col="blue",border="blue")
...
glmd<-lm(poitnsy~pointsx)
conf<-confint(glmd)

plot (pointsx,pointsy,col=pointsc,cex=0.2)
abline(glmd,lty=5,col="darkgreen")
abline(conf[,1],col="grey")
abline(conf[,2],col="grey")
#polygon(,col="grey")

segments(100,100,-100,-100,lty=5,col="darkgreen")
segments(95,105,-105,-95,lty=5,col="grey")
segments(105,95,-95,-105,lty=5,col="grey")
