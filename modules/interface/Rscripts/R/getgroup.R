...
#print(s)
	...
	query<-paste(sep="","select `id`,`seq`,`chr`,`trimmed5`,`trimmed3`,`addition3`,`mut`,`DB`,`freq`,`amb` from `",s,"` ",options,"  `mut` NOT REGEXP '^0[ACTG]+' AND  `mut` NOT REGEXP '^-[0-9]+[ACTG]+';")
	print(query)
...
#  	print(tempn[1:10,])
	temp<-tempn[,c(1,2,3,4,5,6,7,8,9)]
	names(temp)[9]<-"freq"
...
all<-merge(table1[,1:8],table2[,1:8],by=1,all=TRUE)
all[is.na(all)]<-0
